...
LOGS = logging.getLogger("زين جاضد")
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                ...
                buttons=[(Button.url("المطور زين", "https://t.me/devpokemon"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @iiqllll"
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Jepthon.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة اشعارات cr ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
